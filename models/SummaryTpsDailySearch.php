...
/**
 * SummaryTpsDailySearch represents the model behind the search form of `app\models\SummaryTpsDaily`.
 */
class SummaryTpsDailySearch extends SummaryTpsDaily
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $query = SummaryTpsDaily::find();
        if (isset($params['SummaryTpsDailySearch'])) {
            $query = SummaryTpsDaily::find()
                ->select("DATE_FORMAT(time, '%Y-%m-%d')AS `date`, max(mo_tps_max) AS `sms_mo`, max(mt_tps_max) AS `sms_mt`, max(api_tps_max) AS `api`, max(dr_tps_max) AS `dr`")
                ->where(['between','time',$params['SummaryTpsDailySearch']['startDate'].' 00:00:00',$params['SummaryTpsDailySearch']['endDate'].' 23:59:59'])
                // ->andFilterWhere(['site'=>$params['SummaryTpsDaily']['site']])
