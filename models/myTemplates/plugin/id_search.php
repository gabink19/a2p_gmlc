<?
                    if ($field_type_ext_data!=""){
                        ...
                        if (count($split_array)==1) {
                            $columnname=$field_type_ext_data;
                            $field_type_ext_data="";
                        }
                    }
                    ...
                    if ($field_type_ext_data===""){
                            ...
                            foreach($split_array as $split_str){
                                ...
                                if ($no==($max_no-2)) {
                                    ...
                                    if ($no==0) {
                                        ...
                                        $ref_table5=$split_str;
                                    } else {
                                        $ref_table3=$ref_table3.ucfirst($split_str);
                                        $ref_table5=$ref_table5."_".$split_str;
                                    }
                        ...
                        $ref_table5 = str_replace('-','_',$split_array[4]);
                        
                        
                    }
                    $new_label=$new_label."'".strtolower($ref_table_var).$ref_name."' ,";         
                    $var_name= strtolower($ref_table_var).$ref_name."";
                    if (!$virtual_flag) {
                        $join_str=$join_str."//\$query->joinWith('".$ref_table_var." ".$var_name."');\n";
                        $andFilterWhere=$andFilterWhere."//\$query->andFilterWhere(['like', '".$ref_table5.".".$ref_name."', \$this->".$var_name."]);\n";
                    }
