 ...
 */
class XxAphController extends Controller
{
    /**
     * {@inheritdoc}
     */
...
public function actionIndex($app_mode=null)    {
        ...
        if (count($params) >0) {
            $Search_params = $params['AphSearch'];
            if ($Search_params==null) {
                $Search_params=Yii::$app->session['AphSearch'];
                if ($Search_params!=null){        
                    $params['AphSearch']=$Search_params;
                }
            } else {
               Yii::$app->session['AphSearch']=$Search_params;
               
            };
            
            $page_params = $params['page'];
            if ($page_params==null) {
                $page_params=Yii::$app->session['AphSearch_page'];
                if ($page_params!=null){        
                    $_GET['page']=$page_params;
                }
                
            } else {
               Yii::$app->session['AphSearch_page']=$page_params;
            ...
            if ($sort_params==null) {
                $sort_params=Yii::$app->session['AphSearch_sort'];
                if ($sort_params!=null){
                    $_GET['sort']=$sort_params;
                }
                
            } else {
               Yii::$app->session['AphSearch_sort']=$sort_params;
            }
...
public function actionCreate($app_mode=null)     
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            //return $this->redirect(['view', 'id' => $model->ta_id]);
            if ($model->save()) {
...
public function actionCreate2($app_mode=null)    
    ...
    /**
     * Finds the Aph model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Aph the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Aph::findOne($id)) !== null) {
            return $model;
        }
