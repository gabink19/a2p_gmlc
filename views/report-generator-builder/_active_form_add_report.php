  ...
  <div class="tab">
    ...
    <?php 
      foreach ($general_param as $key => $value) {
        echo ucfirst($value).": <p><input placeholder=\"".strtolower($value)."\" type=\"text\" name=\"".strtolower($value)."\" id=\"chart_param_".strtolower($value)."\" style=\"margin-bottom: 15px;\"></p>";
      }
    ?>
  ...
  <div class="tab">
    ...
    <div id="chart_advance" class="collapse">
      <div style="overflow: auto;">
        <div class="col-sm-6" style="display: block; height: 315px;">
          Options:
...
<script>
    ...
    $('#jstree_' + value).jstree({
      ...
      "conditionalselect": function(node, event) {
        ...
        if (value == "advance") {
          ...
          document.getElementById("content" + value).innerHTML += "<button type='button' onclick=\"updateTree(\'" + value + "\',\'" + node.id + "\',\'" + node.parent + "\')\">Add</button>";
        } else if (node["text"] == "filter") {
          document.getElementById("content" + value).innerHTML = "<table id=\"table" + value + "\" style=\"width: 100%; margin-bottom: 10px;\"></table>";
          document.getElementById("table" + value).innerHTML += "<tr><th>select filter</th><td><select id='update_" + value + "' name='addFilter'>"+listcolumn+"</select></td></tr>";
          document.getElementById("content" + value).innerHTML += "<button type='button' onclick=\"updateTree(\'" + value + "\',\'" + node.id + "\',\'" + node.parent + "\')\">Add</button>";
  ...
  function advance() {
    ...
    for(var key in node){var parent=key;var sub=node[key];if(typeof sub=="object"){$('#jstree_advance').jstree().create_node('#',{"id":key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last");for(var key in sub){var parent2=parent+"_"+key;var sub2=sub[key];if(typeof sub2=="object"){$('#jstree_advance').jstree().create_node(""+parent+"",{"id":parent+"_"+key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last");for(var key in sub2){var parent3=parent2+"_"+key;var sub3=sub2[key];if(typeof sub3=="object"){$('#jstree_advance').jstree().create_node(""+parent2+"",{"id":parent2+"_"+key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last");for(var key in sub3){var parent4=parent3+"_"+key;var sub4=sub3[key];if(typeof sub4=="object"){$('#jstree_advance').jstree().create_node(""+parent3+"",{"id":parent3+"_"+key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last");for(var key in sub4){var parent5=parent4+"_"+key;var sub5=sub4[key];if(typeof sub5=="object"){$('#jstree_advance').jstree().create_node(""+parent4+"",{"id":parent4+"_"+key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last");for(var key in sub5){var parent6=parent5+"_"+key;var sub6=sub5[key];if(typeof sub6=="object"){$('#jstree_advance').jstree().create_node(""+parent5+"",{"id":parent5+"_"+key,"text":key,"icon":"glyphicon glyphicon-leaf","state":{"opened":true}},"last")}else{$('#jstree_advance').jstree().create_node(""+parent5+"",{"id":parent5+"_"+key,"text":key+":"+sub5[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}}else{$('#jstree_advance').jstree().create_node(""+parent4+"",{"id":parent4+"_"+key,"text":key+":"+sub4[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}}else{$('#jstree_advance').jstree().create_node(""+parent3+"",{"id":parent3+"_"+key,"text":key+":"+sub3[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}}else{$('#jstree_advance').jstree().create_node(""+parent2+"",{"id":parent2+"_"+key,"text":key+":"+sub2[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}}else{$('#jstree_advance').jstree().create_node(""+parent+"",{"id":parent+"_"+key,"text":key+":"+sub[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}}else{$('#jstree_advance').jstree().create_node('#',{"id":key,"text":key+":"+node[key],"icon":"glyphicon glyphicon-info-sign","state":{"opened":true}},"last")}}
    // modify("advance");
  }
    ...
    if (select == "filter") {
      ...
      $('#jstree_field').jstree().create_node('#', {
        ...
        "children": [{
          ...
          {
            "id": "id_" + node.COLUMN_INDEX + "_filter",
            "text": "filter",
            "icon": "glyphicon glyphicon-info-sign",
            "state": {
              "opened": true
  ...
  function updateTree(select, node, parent) {
    ...
    if (name == "addValue") {
      ...
      $('#jstree_field').jstree().create_node("" + node + "", {
        "id": node + "_" + value2.COLUMN_INDEX,
        "text": value2.ALIAS_NAME,
        "icon": "glyphicon glyphicon-leaf",
        "state": {
          "opened": true
        },
        "children": [{
            "id": node + "_" + value2.COLUMN_INDEX + "_field_no",
            "text": "field_no:" + value2.COLUMN_INDEX,
            "icon": "glyphicon glyphicon-info-sign"
          },
          {
            "id": node + "_" + value2.COLUMN_INDEX + "_data_type",
            "text": "data_type:" + value2.DATA_TYPE,
            "icon": "glyphicon glyphicon-info-sign"
          },
          {
            "id": node + "_" + value2.COLUMN_INDEX + "_operation_type",
            "text": "operation_type:0",
            "icon": "glyphicon glyphicon-info-sign"
          },
          {
            "id": node + "_" + value2.COLUMN_INDEX + "_value",
            "text": "value",
            "icon": "glyphicon glyphicon-menu-hamburger",
            "state": {
              "opened": true
      ...
      for (var key in $('#jstree_field').jstree()._model.data) {
        ...
        if (obj["parent"] == "#") {
          ...
          for (var key in $('#jstree_field').jstree()._model.data) {
            ...
            if (child["parent"] == parent) {
              var split = child["text"].split(":");
              if (child["text"] == "filter") {} else if (isNaN(split[1])) {
                var add = JSON.parse('{"' + split[0] + '":"' + split[1] + '"}')
              } else {
                var add = JSON.parse('{"' + split[0] + '":' + split[1] + '}')
              }
              $.extend(children, add)
            } else if (child["parent"] == parent + "_filter") {
              var parent2 = child["id"];
              var children2 = {};
              children2.value = [];
              for (var key in $('#jstree_field').jstree()._model.data) {
                var child2 = $('#jstree_field').jstree()._model.data[key];
                if (child2["parent"] == parent2) {
                  var split2 = child2["text"].split(":");
                  if (child2["text"] == "value") {} else if (isNaN(split2[1])) {
                    var add2 = JSON.parse('{"' + split2[0] + '":"' + split2[1] + '"}')
                  } else {
                    var add2 = JSON.parse('{"' + split2[0] + '":' + split2[1] + '}')
                  }
                  $.extend(children2, add2)
                } else if (child2["parent"] == parent2 + "_value") {
                  children2.value.push(child2["text"]);
                }
              }
              children.filter.push(children2)
            }
          }
