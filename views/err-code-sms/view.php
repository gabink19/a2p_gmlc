...
<div class="err-code-sms-view">
    ...
    <?if ($update_flag and $remove_update===null) {?>
        ...
        <?= Html::a('Update', ['err-code-sms'."/".$update_action,'app_mode'=>$app_mode, 'id' => $model->tecs_id], ['class' => 'btn btn-primary']) ?>
    <?} else if ($update_flag and $remove_update===2) {?>
        <a class="btn btn-info ErrCodeSmsmodalButton" value="<?= Url::to([$update_action,'app_mode'=>$app_mode, 'id' => $model->tecs_id]) ?>">Update</a>
    <?    }?>
    <?if ($delete_flag and $remove_delete===null) {?>
        <?= Html::a('Delete', ['err-code-sms'."/".$delete_action,'app_mode'=>$app_mode, 'id' => $model->tecs_id], ['class' => 'btn btn-danger']) ?>
    <?} else if ($delete_flag and $remove_delete===2) {?>
        <a class="btn btn-info ErrCodeSmsajaxButton" value="<?= Url::to([$delete_action,'app_mode'=>$app_mode, 'id' => $model->tecs_id]) ?>">Delete</a>
    <?    }?>
